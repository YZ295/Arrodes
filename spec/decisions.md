...
# 架构决策：多 Agent 工作区（workspace 完善）

> grill-me 阶段，2026-08-15

## D1｜Agent 对话适配方式

- 状态：accepted（2026-08-15；用户对细化版未提异议并继续确认后续决策）
- 备选：
  1. 一次性委派 + Agent 适配器注册表（推荐）：每种 agent 一个适配器实现统一 `AgentChatAdapter` 接口（`run(task, { history, cwd })`）——codex → `codex exec`、hermes → `hermes` CLI、未来 agent 新增适配器即可；每次对话 = 一次非交互调用，历史拼入任务描述。
  2. 常驻会话桥：每 agent 维护一个交互式子进程，多轮自然对话。
  3. 只派任务不聊天。
- 结论：一次性委派 + Agent 适配器注册表（多 agent 可扩展）。

## D2｜「连线」语义

- 状态：accepted（2026-08-15，用户确认 D2.A）
- 结论：连线 = 把 agent 接入工作区成员 + 全量共享记忆；共享永远是全部，不做部分过滤（避免部分匹配出错）。

## D3｜记忆统一写入入口

- 状态：accepted（2026-08-15，用户确认 D3.A）
- 结论：Arrodes 中转——外部 agent 经 Arrodes REST/工具写入工作区记忆，由 Arrodes 统一格式并全量同步 Obsidian；外部 agent 不直接写 Obsidian。

## D4｜工作区粒度

- 状态：accepted（2026-08-15，用户确认 D4.A）
- 结论：项目级容器——工作区按项目隔离，default + 每项目一个，记忆/agent/任务都挂在工作区下。

## D5｜对话 UI 形态

- 状态：accepted（2026-08-15，用户确认 D5.A）
- 结论：面板内嵌聊天——工作区面板选中 agent 右侧聊天窗，多轮对话，记录挂在工作区下。

## D6｜布局形态

- 状态：accepted（2026-08-15，用户确认「先做 A，测试完后做 B」）
- 结论：先做 A（Agent 卡片 + 连线按钮），测试验收后再升级 B（自由拖拽画布 + SVG 连线）。

## 汇总

多 Agent 工作区：Agent 适配器注册表（一次性委派）→ 面板内嵌聊天；连线 = 接入 + 全量共享记忆；记忆由 Arrodes 统一格式写入 Obsidian；按项目级工作区隔离；卡片+连线先行、画布后续。
